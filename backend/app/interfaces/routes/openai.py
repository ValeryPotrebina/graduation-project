...
async def _format_materials_for_context(
    ...
    for i, material_item in enumerate(materials):
        ...
        if material_item.content:
            content_preview = strings.CONTEXT_MATERIAL_CONTENT_PREVIEW.format(
                content_preview=material_item.content[:50])

        output_parts.append(
            f"  {i + 1}. {material_name_display}{content_preview}")
    return output_parts
...
async def get_context_from_db(
    ...
    teacher_requested = any(
        kw in normalized_question for kw in strings.KEYWORDS_TEACHER)
    description_requested = any(
        kw in normalized_question for kw in strings.KEYWORDS_DESCRIPTION)
    labs_requested = any(
        kw in normalized_question for kw in strings.KEYWORDS_LABS)
    hours_requested = any(
        kw in normalized_question for kw in strings.KEYWORDS_HOURS)
    lectures_requested = any(
        kw in normalized_question for kw in strings.KEYWORDS_LECTURES)
    seminars_requested = any(
        kw in normalized_question for kw in strings.KEYWORDS_SEMINARS)
    all_materials_requested = any(
        kw in normalized_question for kw in strings.KEYWORDS_ALL_MATERIALS)
    ...
    sorted_courses = sorted(all_db_courses, key=lambda c: len(
        c.name) if c.name else 0, reverse=True)
    ...
    if found_db_course:
        ...
        context_parts.append(
            strings.CONTEXT_COURSE_INFO.format(course_name=course_name))

        is_general_query = not (
            teacher_requested or description_requested or labs_requested or hours_requested)
        ...
        if teacher_requested or is_general_query:
            ...
            context_parts.append(strings.CONTEXT_TEACHER_INFO.format(
                teacher_info=display_teacher))

        if description_requested or is_general_query:
            if found_db_course.description:
                context_parts.append(strings.CONTEXT_DESCRIPTION_INFO.format(
                    description=found_db_course.description))
        ...
        if hours_requested or is_general_query:
            if found_db_course.hours is not None:
                context_parts.append(strings.CONTEXT_HOURS_INFO.format(
                    hours=found_db_course.hours))
        ...
        if labs_requested or lectures_requested or seminars_requested or all_materials_requested:
            ...
            if found_db_course.id is not None:
                ...
                async def fetch_and_format_materials(material_db_type_to_fetch: str):
                    ...
                    try:
                        ...
                        if all_course_materials is not None:
                            specific_materials = [
                                m for m in all_course_materials if m.material_type == material_db_type_to_fetch]
                        ...
                        material_type_rus_plural, _ = strings.MATERIAL_TYPE_TRANSLATIONS_RUS.get(
                            material_db_type_to_fetch, ("материалы", ""))
                ...
                if all_materials_requested and len(context_parts) <= 1:
                    context_parts.append(
                        strings.CONTEXT_ALL_MATERIALS_INTRO.format(course_name=course_name))
                ...
                is_material_keywords_present = any(kw in normalized_question for kw in strings.KEYWORDS_ALL_MATERIALS +
                                                   strings.KEYWORDS_LECTURES + strings.KEYWORDS_SEMINARS + strings.KEYWORDS_LABS)
                if is_material_keywords_present and not materials_context_generated and not is_general_query_for_course_details:
                    context_parts.append(
                        strings.CONTEXT_NO_SPECIFIC_MATERIALS_REQUESTED.format(course_name=course_name))

            elif labs_requested or lectures_requested or seminars_requested or all_materials_requested:
                context_parts.append(strings.CONTEXT_COURSE_ID_FOR_MATERIALS_NOT_FOUND.format(
                    course_name=course_name))
    else:
...
async def ask_openai(
    ...
    try:
        ...
        answer_text = response.choices[0].message.content if response.choices and response.choices[
            0].message else strings.OPENAI_DEFAULT_ERROR_RESPONSE
        ...
        raise HTTPException(
            status_code=500,
            detail=strings.HTTP_DETAIL_INTERNAL_SERVER_ERROR_OPENAI.format(
                error_message=str(e))
        )
